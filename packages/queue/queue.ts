...
export class ServerlessQueue {
   ...
   */
  static async add<T extends Record<string, unknown>>(
    type: string,
    payload: T,
    options: JobOptions = {},
  ): Promise<string> {
   ...
   */
  static async addBulk<T extends Record<string, unknown>>(
   ...
   */
  static async process(limit = 10): Promise<number> {
   ...
   */
  private static async executeJob(job: JobQueue): Promise<void> {
